<script lang="ts">
	...
	onMount(() => {
		...
		const onShare = async (e: MouseEvent) => {
			...
			// button. url only: a `text` gets pasted in front of the link in most apps, and the link
			// already unfurls into a card naming the place. Cancelling the sheet throws AbortError
			// and must stay a no-op: falling back to a copy there would claim to have done something
			...
			for (const p of PLACES) {
				...
				const m = L.marker(p.pin ?? [p.lat, p.lng], { icon }).bindPopup(
					`<div class="pop"><div class="pop-type" style="color:${col(p)}">${esc(p.type)}</div>` +
						...
						`<button class="sharebtn" type="button" data-share="${slug(p)}" title="Share ${esc(p.name)}" aria-label="Share ${esc(p.name)}">` +
